                      ...
                      <div class="box-body">
                        ...
                        <div class="col-xs-4"><label>No. Invoice</label>
                         <select name="kategori" id="kategori" class="form-control" required>
                            ...
                            <?php
                            $query = "SELECT id_trans FROM trans_jual WHERE id_member ='$sesen_id' AND status = 2 ORDER BY id_trans '";
                            $sql = mysqli_query($koneksi, $query);
                            while($data = mysqli_fetch_array($sql)){echo '<option value="'.$data['id_trans'].'">'.$data['id_trans'].'</option>';}
                            ?>?>
                        </select>
                        </div>
