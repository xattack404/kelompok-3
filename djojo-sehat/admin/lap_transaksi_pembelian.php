...
if (isset($_POST['tampilkan'])) {
	...
	if ($cek == 1) {
		...
		$total = "SELECT sum(subtotal) as subtotal, sum(jumlah) as jumlah FROM detail_beli";
	...
	if($cek==2){
		...
		$total = "SELECT sum(subtotal) as subtotal, sum(jumlah) as jumlah FROM detail_beli INNER JOIN trans_beli ON trans_beli.id_beli = detail_beli.id_beli WHERE trans_beli.tanggal>='$tgl_awal' AND trans_beli.tanggal <='$tgl_akhir' ORDER BY tanggal asc";
	}
}
